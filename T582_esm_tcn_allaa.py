...
logging.basicConfig(filename='Log/T582_esm(150M)_tcn_allaa_floss.log', level=logging.INFO, format='%(asctime)s - %(message)s')
...
def cross_validate_model(file, batch_converter, esm_model, epochs, patience):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(sequences)):
        ...
        print("******************* StepLR : step_size=20, gamma=0.1 *********************")
        logging.info("******************* StepLR : step_size=20, gamma=0.1 *********************")

        model_path = f'Best/fold_{fold + 1}_full_(150m)_T2_tcn_all_lr.pth'
...
if __name__ == "__main__":
    ...
    model_paths = [f'Best/fold_{i + 1}_full_(150m)_T2_tcn_all_lr.pth' for i in range(5)]
